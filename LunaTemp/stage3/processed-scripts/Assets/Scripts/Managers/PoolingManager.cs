...
public class PoolingManager : MonoBehaviour
{
   public static PoolingManager Instance {get; private set;}
    ...
    private GameObject EnemySpawn(ParticlePool Pool, [Bridge.Ref] Vector3 Pos,  [Bridge.Ref] Quaternion rot, float duration = 0)
    {
    ...
    private GameObject Play(ParticlePool Pool, [Bridge.Ref] Vector3 Pos, Transform parent)
    {
        if(Pool.ParticleQueue.Count > 0)
        {
    ...
    private GameObject Play(ParticlePool Pool, [Bridge.Ref] Vector3 pos, float dequeDuration, Transform Parent = null)
    {
        if(Pool.ParticleQueue.Count > 0)
        {
            ParticleSystem PS = Pool.ParticleQueue.Dequeue().GetComponent<ParticleSystem>();
    ...
    private void Play(ParticlePool Pool, [Bridge.Ref] Vector3 pos)
    {
        if(Pool.ParticleQueue.Count > 0)
        {
            ParticleSystem PS = Pool.ParticleQueue.Dequeue().GetComponent<ParticleSystem>();
