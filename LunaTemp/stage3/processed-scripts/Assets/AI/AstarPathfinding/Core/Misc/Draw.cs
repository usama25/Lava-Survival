...
namespace Pathfinding.Util {
	...
	public class Draw {
		...
		void SetColor ([Bridge.Ref] Color color) {
			if (gizmos && UnityEngine.Gizmos.color != color) UnityEngine.Gizmos.color = color;
		}

		public void Polyline (System.Collections.Generic.List<Vector3> points, [Bridge.Ref] Color color, bool cycle = false) {
		...
		public void Line ([Bridge.Ref] Vector3 a, [Bridge.Ref] Vector3 b, [Bridge.Ref] Color color) {
			SetColor(color);
		...
		public void CircleXZ ([Bridge.Ref] Vector3 center, float radius, [Bridge.Ref] Color color, float startAngle = 0f, float endAngle = 2*Mathf.PI) {
			int steps = 40;
		...
		public void Cylinder ([Bridge.Ref] Vector3 position, [Bridge.Ref] Vector3 up, float height, float radius, [Bridge.Ref] Color color) {
		...
		public void CrossXZ ([Bridge.Ref] Vector3 position, [Bridge.Ref] Color color, float size = 1) {
			size *= 0.5f;
		...
		public void Bezier ([Bridge.Ref] Vector3 a, [Bridge.Ref] Vector3 b, [Bridge.Ref] Color color) {
			Vector3 dir = b - a;
