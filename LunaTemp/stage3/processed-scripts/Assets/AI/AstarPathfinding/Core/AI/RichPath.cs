...
namespace Pathfinding {
	public class RichPath {
		...
		/// </summary>
		public void GetRemainingPath (List<Vector3> buffer, [Bridge.Ref] Vector3 currentPosition, out bool requiresRepath) {
			buffer.Clear();
	...
	public class RichFunnel : RichPathPart {
		...
		/// </summary>
		void SimplifyPath (IRaycastableGraph graph, List<GraphNode> nodes, int start, int end, List<GraphNode> result, [Bridge.Ref] Vector3 startPoint, [Bridge.Ref] Vector3 endPoint) {
			if (graph == null) throw new System.ArgumentNullException("graph");
		...
		/// </summary>
		public Vector3 ClampToNavmesh ([Bridge.Ref] Vector3 position) {
		...
		public Vector3 Update ([Bridge.Ref] Vector3 position, List<Vector3> buffer, int numCorners, out bool lastCorner, out bool requiresRepath) {
			if (path.transform != null) position = path.transform.InverseTransform(position);
		...
		bool FindNextCorners ([Bridge.Ref] Vector3 origin, int startIndex, List<Vector3> funnelPath, int numCorners, out bool lastCorner) {
			lastCorner = false;
