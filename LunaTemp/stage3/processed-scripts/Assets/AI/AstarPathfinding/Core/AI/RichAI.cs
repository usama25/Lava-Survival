...
namespace Pathfinding {
	...
	public partial class RichAI : AIBase, IAstarAI {
		...
		/// </summary>
		public override void Teleport ([Bridge.Ref] Vector3 newPosition, bool clearPath = true) {
		...
		void FinalMovement ([Bridge.Ref] Vector3 position3D, float deltaTime, float distanceToEndOfPath, float slowdownFactor, out Vector3 nextPosition, out Quaternion nextRotation) {
			var forwards = movementPlane.ToPlane(simulatedRotation * (orientation == OrientationMode.YAxisForward ? Vector3.up : Vector3.forward));
		...
		protected override Vector3 ClampToNavmesh ([Bridge.Ref] Vector3 position, out bool positionChanged) {
			if (richPath != null) {
		...
		Vector2 CalculateWallForce ([Bridge.Ref] Vector2 position, float elevation, [Bridge.Ref] Vector2 directionToTarget) {
