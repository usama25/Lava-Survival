...
namespace Pathfinding {
	...
	public abstract class AIBase : VersionedMonoBehaviour {
		...
		public virtual void Teleport ([Bridge.Ref] Vector3 newPosition, bool clearPath = true) {
		...
		protected Vector2 CalculateDeltaToMoveThisFrame ([Bridge.Ref] Vector2 position, float distanceToEndOfPath, float deltaTime) {
		...
		public Quaternion SimulateRotationTowards ([Bridge.Ref] Vector3 direction, float maxDegrees) {
		...
		/// <param name="maxDegrees">Maximum number of degrees to rotate this frame.</param>
		protected Quaternion SimulateRotationTowards ([Bridge.Ref] Vector2 direction, float maxDegrees) {
		...
		public virtual void Move ([Bridge.Ref] Vector3 deltaPosition) {
		...
		public virtual void FinalizeMovement ([Bridge.Ref] Vector3 nextPosition, [Bridge.Ref] Quaternion nextRotation) {
			if (enableRotation) FinalizeRotation(nextRotation);
			FinalizePosition(nextPosition);
		}

		void FinalizeRotation ([Bridge.Ref] Quaternion nextRotation) {
		...
		void FinalizePosition ([Bridge.Ref] Vector3 nextPosition) {
		...
		protected virtual Vector3 ClampToNavmesh ([Bridge.Ref] Vector3 position, out bool positionChanged) {
		...
		protected Vector3 RaycastPosition ([Bridge.Ref] Vector3 position, float lastElevation) {
			RaycastHit hit;
			float elevation;
