...
namespace Pathfinding {
	...
	public class Funnel {
		...
		public static FunnelPortals ConstructFunnelPortals (List<GraphNode> nodes, [Bridge.Ref] PathPart part) {
			if (nodes == null || nodes.Count == 0) {
		...
		public static void ShrinkPortals ([Bridge.Ref] FunnelPortals portals, float shrink) {
		...
		static bool UnwrapHelper ([Bridge.Ref] Vector3 portalStart, [Bridge.Ref] Vector3 portalEnd, [Bridge.Ref] Vector3 prevPoint, [Bridge.Ref] Vector3 nextPoint, ref Quaternion mRot, ref Vector3 mOffset) {
		...
		/// </summary>
		public static void Unwrap ([Bridge.Ref] FunnelPortals funnel, Vector2[] left, Vector2[] right) {
			int startingIndex = 1;
		...
		protected static Vector2 ToXZ ([Bridge.Ref] Vector3 p) {
			return new Vector2(p.x, p.z);
		}

		protected static Vector3 FromXZ ([Bridge.Ref] Vector2 p) {
			return new Vector3(p.x, 0, p.y);
		}

		/// <summary>True if b is to the right of or on the line from (0,0) to a</summary>
		protected static bool RightOrColinear ([Bridge.Ref] Vector2 a, [Bridge.Ref] Vector2 b) {
			return (a.x*b.y - b.x*a.y) <= 0;
		}

		/// <summary>True if b is to the left of or on the line from (0,0) to a</summary>
		protected static bool LeftOrColinear ([Bridge.Ref] Vector2 a, [Bridge.Ref] Vector2 b) {
			return (a.x*b.y - b.x*a.y) >= 0;
		}
		...
		public static List<Vector3> Calculate ([Bridge.Ref] FunnelPortals funnel, bool unwrap, bool splitAtEveryPortal) {
