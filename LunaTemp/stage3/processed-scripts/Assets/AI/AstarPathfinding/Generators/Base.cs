...
namespace Pathfinding {
	...
	public abstract class NavGraph : IGraphInternals {
		...
		public virtual bool IsInsideBounds ([Bridge.Ref] Vector3 point) {
			return true;
		}
		...
		/// </summary>
		[System.Obsolete("Use the transform field (only available on some graph types) instead", true)]
		public void SetMatrix ([Bridge.Ref] Matrix4x4 m) {
			matrix = m;
		...
		public void RelocateNodes ([Bridge.Ref] Matrix4x4 oldMatrix, [Bridge.Ref] Matrix4x4 newMatrix) {
		...
		/// </summary>
		public virtual void RelocateNodes ([Bridge.Ref] Matrix4x4 deltaMatrix) {
		...
		public NNInfoInternal GetNearest ([Bridge.Ref] Vector3 position) {
		...
		public NNInfoInternal GetNearest ([Bridge.Ref] Vector3 position, NNConstraint constraint) {
		...
		/// <param name="constraint">Can for example tell the function to try to return a walkable node. If you do not get a good node back, consider calling GetNearestForce.</param>
		public virtual NNInfoInternal GetNearest ([Bridge.Ref] Vector3 position, NNConstraint constraint, GraphNode hint) {
		...
		/// </summary>
		public virtual NNInfoInternal GetNearestForce ([Bridge.Ref] Vector3 position, NNConstraint constraint) {
	...
	public class GraphCollision {
		...
		/// </summary>
		public bool Check ([Bridge.Ref] Vector3 position) {
			if (!collisionCheck) {
				return true;
			}
		...
		/// </summary>
		public Vector3 CheckHeight ([Bridge.Ref] Vector3 position) {
			RaycastHit hit;
			bool walkable;
		...
		/// </summary>
		public Vector3 CheckHeight ([Bridge.Ref] Vector3 position, out RaycastHit hit, out bool walkable) {
			walkable = true;
		...
		/// </summary>
		public RaycastHit[] CheckHeightAll ([Bridge.Ref] Vector3 position, out int numHits) {
			if (!heightCheck || use2D) {
