...
namespace Pathfinding {
	...
	public class LayerGridGraph : GridGraph, IUpdatableGraph {
		...
		class HitComparer : IComparer<RaycastHit> {
			public int Compare ([Bridge.Ref] RaycastHit a, [Bridge.Ref] RaycastHit b) {
		...
		/// See: GraphCollision.
		/// </summary>
		protected static HeightSample[] SampleHeights (GraphCollision collision, float mergeSpanRange, [Bridge.Ref] Vector3 position, out int numHits) {
			int raycastHits;
		...
		protected override GridNodeBase GetNearestFromGraphSpace ([Bridge.Ref] Vector3 positionGraphSpace) {
			if (nodes == null || depth*width*layerCount != nodes.Length) {
				return null;
			}
		...
		private GridNodeBase GetNearestNode ([Bridge.Ref] Vector3 position, int x, int z, NNConstraint constraint) {
			int index = width*z+x;
	...
	public class LevelGridNode : GridNodeBase {
		...
		public void SetPosition ([Bridge.Ref] Int3 position) {
		...
		public override Vector3 ClosestPointOnNode ([Bridge.Ref] Vector3 p) {
			var gg = GetGridGraph(GraphIndex);
