...
namespace Pathfinding.Recast {
	...
	internal class RecastMeshGatherer {
		...
		static List<MeshFilter> FilterMeshes (MeshFilter[] meshFilters, List<string> tagMask, [Bridge.Ref] LayerMask layerMask) {
		...
		RasterizationMesh GenerateHeightmapChunk (float[, ] heights, [Bridge.Ref] Vector3 sampleSize, [Bridge.Ref] Vector3 offset, int x0, int z0, int width, int depth, int stride) {
		...
		/// </summary>
		RasterizationMesh RasterizeCollider (Collider col, [Bridge.Ref] Matrix4x4 localToWorldMatrix) {
		...
		RasterizationMesh RasterizeBoxCollider (BoxCollider collider, [Bridge.Ref] Matrix4x4 localToWorldMatrix) {
		...
		RasterizationMesh RasterizeCapsuleCollider (float radius, float height, [Bridge.Ref] Bounds bounds, [Bridge.Ref] Matrix4x4 localToWorldMatrix) {
