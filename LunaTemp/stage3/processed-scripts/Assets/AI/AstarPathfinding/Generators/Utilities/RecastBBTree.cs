...
namespace Pathfinding {
	...
	public class RecastBBTree {
		...
		public void QueryInBounds ([Bridge.Ref] Rect bounds, List<RecastMeshObj> buffer) {
			if (root == null) return;
			QueryBoxInBounds(root, bounds, buffer);
		}

		void QueryBoxInBounds (RecastBBTreeBox box, [Bridge.Ref] Rect bounds, List<RecastMeshObj> boxes) {
			if (box.mesh != null) {
				// Leaf node
		...
		RecastBBTreeBox RemoveBox (RecastBBTreeBox c, RecastMeshObj mesh, [Bridge.Ref] Rect bounds, ref bool found) {
		...
		static bool RectIntersectsRect ([Bridge.Ref] Rect r, [Bridge.Ref] Rect r2) {
		...
		static float ExpansionRequired ([Bridge.Ref] Rect r, [Bridge.Ref] Rect r2) {
		...
		static Rect ExpandToContain ([Bridge.Ref] Rect r, [Bridge.Ref] Rect r2) {
			float xMin = Mathf.Min(r.xMin, r2.xMin);
			float xMax = Mathf.Max(r.xMax, r2.xMax);
			float yMin = Mathf.Min(r.yMin, r2.yMin);
			float yMax = Mathf.Max(r.yMax, r2.yMax);
		...
		static float RectArea ([Bridge.Ref] Rect r) {
	...
	public class RecastBBTreeBox {
		...
		public bool Contains ([Bridge.Ref] Vector3 p) {
