...
namespace Pathfinding.Util {
	...
	public class GraphTransform : IMovementPlane, ITransform {
		...
		public Vector3 WorldUpAtGraphPosition ([Bridge.Ref] Vector3 point) {
			return up;
		}
		...
		public Vector3 Transform ([Bridge.Ref] Vector3 point) {
			if (onlyTranslational) return point + translation;
			return matrix.MultiplyPoint3x4(point);
		}

		public Vector3 TransformVector ([Bridge.Ref] Vector3 point) {
		...
		public Vector3 InverseTransform ([Bridge.Ref] Vector3 point) {
			if (onlyTranslational) return point - translation;
			return inverseMatrix.MultiplyPoint3x4(point);
		}

		public Vector3 InverseTransformVector ([Bridge.Ref] Vector3 dir) {
			if (onlyTranslational) return dir;
			return inverseMatrix.MultiplyVector(dir);
		}

		public Int3 InverseTransform ([Bridge.Ref] Int3 point) {
		...
		public Bounds Transform ([Bridge.Ref] Bounds bounds) {
		...
		public Bounds InverseTransform ([Bridge.Ref] Bounds bounds) {
			if (onlyTranslational) return new Bounds(bounds.center - translation, bounds.size);
		...
		/// </summary>
		Vector2 IMovementPlane.ToPlane ([Bridge.Ref] Vector3 point) {
		...
		/// The transformation is purely a rotation so no scale or offset is used.
		/// </summary>
		Vector2 IMovementPlane.ToPlane ([Bridge.Ref] Vector3 point, out float elevation) {
			if (!isXZ) point = inverseRotation * point;
			elevation = point.y;
			return new Vector2(point.x, point.z);
		}
		...
		/// The transformation is purely a rotation so no scale or offset is used.
		/// </summary>
		Vector3 IMovementPlane.ToWorld ([Bridge.Ref] Vector2 point, float elevation) {
