...
namespace Pathfinding {
	...
	public class BBTree : IAstarPooledObject {
		...
		int GetBox ([Bridge.Ref] IntRect rect) {
		...
		static void DrawDebugRect ([Bridge.Ref] IntRect rect) {
		...
		[System.Diagnostics.Conditional("ASTARDEBUG")]
		static void DrawDebugNode (TriangleMeshNode node, float yoffset, [Bridge.Ref] Color color) {
		...
		/// </summary>
		public NNInfoInternal QueryClosest ([Bridge.Ref] Vector3 p, NNConstraint constraint, out float distance) {
		...
		public NNInfoInternal QueryClosestXZ ([Bridge.Ref] Vector3 p, NNConstraint constraint, ref float distance, [Bridge.Ref] NNInfoInternal previous) {
		...
		void SearchBoxClosestXZ (int boxi, [Bridge.Ref] Vector3 p, ref float closestSqrDist, NNConstraint constraint, ref NNInfoInternal nnInfo) {
		...
		public NNInfoInternal QueryClosest ([Bridge.Ref] Vector3 p, NNConstraint constraint, ref float distance, [Bridge.Ref] NNInfoInternal previous) {
			var sqrDistance = distance*distance;
			var origSqrDistance = sqrDistance;
		...
		void SearchBoxClosest (int boxi, [Bridge.Ref] Vector3 p, ref float closestSqrDist, NNConstraint constraint, ref NNInfoInternal nnInfo) {
			BBTreeBox box = tree[boxi];
		...
		void GetOrderedChildren (ref int first, ref int second, out float firstDist, out float secondDist, [Bridge.Ref] Vector3 p) {
		...
		/// </summary>
		public TriangleMeshNode QueryInside ([Bridge.Ref] Vector3 p, NNConstraint constraint) {
			return count != 0 && tree[0].Contains(p) ? SearchBoxInside(0, p, constraint) : null;
		}

		TriangleMeshNode SearchBoxInside (int boxi, [Bridge.Ref] Vector3 p, NNConstraint constraint) {
			BBTreeBox box = tree[boxi];
		...
		struct BBTreeBox {
			...
			public bool Contains ([Bridge.Ref] Vector3 point) {
				var pi = (Int3)point;
		...
		static bool NodeIntersectsCircle (TriangleMeshNode node, [Bridge.Ref] Vector3 p, float radius) {
		...
		/// </summary>
		static bool RectIntersectsCircle ([Bridge.Ref] IntRect r, Vector3 p, float radius) {
			if (float.IsPositiveInfinity(radius)) return true;
		...
		static float SquaredRectPointDistance ([Bridge.Ref] IntRect r, Vector3 p) {
			Vector3 po = p;
