...
namespace Pathfinding {
	...
	public abstract class NavmeshBase : NavGraph, INavmesh, INavmeshHolder, ITransformedGraph
		...
		/// </summary>
		public Bounds GetTileBounds ([Bridge.Ref] IntRect rect) {
		...
		public Bounds GetTileBoundsInGraphSpace ([Bridge.Ref] IntRect rect) {
		...
		/// </summary>
		public IntRect GetTouchingTiles ([Bridge.Ref] Bounds bounds, float margin = 0) {
		...
		public IntRect GetTouchingTilesInGraphSpace ([Bridge.Ref] Rect rect) {
			// Calculate world bounds of all affected tiles
		...
		/// </summary>
		public IntRect GetTouchingTilesRound ([Bridge.Ref] Bounds bounds) {
			bounds = transform.InverseTransform(bounds);
		...
		/// See: GetNearest
		/// </summary>
		public GraphNode PointOnNavmesh ([Bridge.Ref] Vector3 position, NNConstraint constraint) {
		...
		/// [Open online documentation to see images]
		/// </summary>
		public bool Linecast ([Bridge.Ref] Vector3 origin, [Bridge.Ref] Vector3 end) {
		...
		public bool Linecast ([Bridge.Ref] Vector3 origin, [Bridge.Ref] Vector3 end, GraphNode hint, out GraphHitInfo hit) {
		...
		/// <param name="hint">You may pass the node closest to the start point if you already know it for a minor performance boost.
		/// 			   If null, a search for the closest node will be done. This parameter is mostly deprecated and should be avoided. Pass null instead.</param>
		public bool Linecast ([Bridge.Ref] Vector3 origin, [Bridge.Ref] Vector3 end, GraphNode hint) {
			GraphHitInfo hit;
		...
		public bool Linecast ([Bridge.Ref] Vector3 origin, [Bridge.Ref] Vector3 end, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace) {
		...
		public bool Linecast ([Bridge.Ref] Vector3 origin, [Bridge.Ref] Vector3 end, out GraphHitInfo hit, List<GraphNode> trace, System.Func<GraphNode, bool> filter) {
		...
		/// <param name="hint">You may pass the node closest to the start point if you already know it for a minor performance boost.
		/// 			   If null, a search for the closest node will be done. This parameter is mostly deprecated and should be avoided. Pass null instead.</param>
		public static bool Linecast (NavmeshBase graph, [Bridge.Ref] Vector3 origin, [Bridge.Ref] Vector3 end, GraphNode hint, out GraphHitInfo hit) {
		...
		/// <param name="filter">If not null then the delegate will be called for each node and if it returns false the node will be treated as unwalkable and a hit will be returned.
		///               Note that unwalkable nodes are always treated as unwalkable regardless of what this filter returns.</param>
		public static bool Linecast (NavmeshBase graph, [Bridge.Ref] Vector3 origin, [Bridge.Ref] Vector3 end, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace, System.Func<GraphNode, bool> filter = null) {
